...
if (isset($_GET['search']) && !empty($_GET['search'])) {
    ...
    $sql = "SELECT * FROM contacts WHERE title LIKE '%$searchQuery%' OR description LIKE '%$searchQuery%' LIMIT $records_per_page OFFSET $offset";
} else {
    $sql = "SELECT * FROM contacts LIMIT $records_per_page OFFSET $offset";
}

$result = $conn->query($sql);

$total_result_sql = "SELECT COUNT(*) AS total FROM contacts";
if (!empty($searchQuery)) {
    $total_result_sql = "SELECT COUNT(*) AS total FROM contacts WHERE title LIKE '%$searchQuery%' OR description LIKE '%$searchQuery%'";
}
...
<div class="position-admin">
    ...
    <div class="position-table">
        <div class="table-sup">
            <div class="title-table">Contact Table</div>
            ...
            <table>
                ...
                if ($result->num_rows > 0) {
                    while($row = $result->fetch_assoc()) {
                        ...
                        echo '<td>' . $row['date'] . '</td>';
                        echo '<td><a href="delete-mess.php?id=' . $row['id'] . '">Delete</a></td>';
                        echo '</tr>';
                    }
                } else {
