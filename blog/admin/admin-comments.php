...
if (isset($_GET['search']) && !empty($_GET['search'])) {
    ...
    $sql = "SELECT comments.id, comments.text, comments.post_id, comments.date, users.username AS author_name 
            FROM comments 
            ...
            <table>
                ...
                if ($result->num_rows > 0) {
                    while($row = $result->fetch_assoc()) {
                        ...
                        echo '<td>' . $row['date'] . '</td>';
                        echo '<td><a href="delete-comment.php?id=' . $row['id'] . '">Delete</a></td>';
                        echo '</tr>';
                    }
                } else {
